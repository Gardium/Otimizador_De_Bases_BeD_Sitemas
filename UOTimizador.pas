...
uses
  ...
  ZAbstractRODataset, ZAbstractDataset, ZDataset, DateUtils, ZipMstr, ComCtrls;
      ...
      if Months >=0  then
        ...
        {Leitura do Logo.ini}
          begin
            ...
            ipServ := ArquivoINI.ReadString('SERVIDOR', 'ip', ipServ);
            if ip = ipServ then
              db := ArquivoINI.ReadString('SERVIDOR', 'db', DB);
        ...
        {Conectando ao Banco ultilizando Zeos}
         ...
         {Retornando id_nfcevendas}
          begin
            ZQuery1.SQL.Text :=
              'UPDATE  tbl_nfcevendas  SET xml = ""  WHERE curdate() - data  >= 300 and xml is not null';
            ZQuery1.Active;
            ZQuery1.ExecSQL;
            ZQuery1.Close;
...
begin
  MessageDlg('Base otimizada com sucesso ',mtInformation,[mbOk],0);
             {Fechando Form}
  Form1.Close;
end;
